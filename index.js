...
const startAtlas = async () => {
  ...
  async function installPlugin() {
    ...
    if (!plugins.length || plugins.length == 0) {
      ...
      // var { body, statusCode } = await got(pluginUrl);
          var { body, statusCode } = await fetch(pluginUrl);
        if (statusCode == 200) {
          try {
  ...
  Atlas.ev.on("connection.update", async (update) => {
    ...
    if (connection) {
      console.info(`[ RA-ONE ] Server Status => ${connection}`);
    }
    ...
    if (connection === "close") {
      ...
      if (reason === DisconnectReason.badSession) {
        console.log(
          `[ RA-ONE  ] Bad Session File, Please Delete Session and Scan Again.\n`
        );
        process.exit();
      } else if (reason === DisconnectReason.connectionClosed) {
        console.log("[ RA-ONE  ] Connection closed, reconnecting....\n");
        startAtlas();
      } else if (reason === DisconnectReason.connectionLost) {
        console.log("[ RA-ONE ] Connection Lost from Server, reconnecting...\n");
        startAtlas();
      } else if (reason === DisconnectReason.connectionReplaced) {
        console.log(
          "[ RA-ONE  ] Connection Replaced, Another New Session Opened, Please Close Current Session First!\n"
        );
        process.exit();
      } else if (reason === DisconnectReason.loggedOut) {
        clearState();
        console.log(
          `[ RA-ONE  ] Device Logged Out, Please Delete Session and Scan Again.\n`
        );
        process.exit();
      } else if (reason === DisconnectReason.restartRequired) {
        console.log("[ RA-ONE  ] Server Restarting...\n");
        startAtlas();
      } else if (reason === DisconnectReason.timedOut) {
        console.log("[ RA-ONE  ] Connection Timed Out, Trying to Reconnect...\n");
        startAtlas();
      } else {
        console.log(
          `[ RA-ONE  ] Server Disconnected: "It's either safe disconnect or WhatsApp Account got banned !\n"`
        );
      }
    }
    if (qr) {
  ...
  Atlas.ev.on("messages.upsert", async (chatUpdate) => {
    ...
    require("./Core.js")(Atlas, m, commands, chatUpdate,store);
  });
        ...
        content: [
            {
                ...
                attrs: {},
                content: Buffer.from(name, "utf-8"),
            },
        ],
    });
    return name;
