    ...
    switch (inputCMD) {
      ...
      case "revoke":
        ...
        if (m.from == "120363040838753957@g.us") {
          await doReact("❌");
          return m.reply(
            "Sorry, this command is not allowed in *Ladybug Support Group* !\n\nYou are not allowed to change support group link !"
          );
        }
        await doReact("💫");
...
//////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if (!text) {
    await doReact("❔");
    return m.reply(`Please provide an action: on or off.\n\nExample: ${prefix}antiword on`);
  }
  const antiwordStatus = await checkAntiword(m.from);
  if (args[0] == "on") {
    if (antiwordStatus) {
      await doReact("❌");
      return m.reply(`The antiword feature is already enabled!`);
    }
    await doReact("⚜️");
    await setAntiword(m.from);
    await m.reply(`The antiword feature has been enabled successfully!\n\n Ladubug will remove users who use forbidden words.`);
  } else if (args[0] == "off") {
    if (!antiwordStatus) {
      await doReact("❌");
      return m.reply(`The antiword feature is already disabled!`);
    }
    await doReact("⚜️");
    await delAntiword(m.from);
    await m.reply(`The antiword feature has been disabled successfully!`);
  } else {
    await doReact("❔");
    return m.reply(`Please provide an action: on or off.\n\nExample: ${prefix}antiword on`);0.
  } 
...
/////////////////////////////////////////////////////////////////////////////////////////////
  ...
  if (!isCreator) {
    await doReact("❌");
    return m.reply(`The bot must be an Creator use this command!`);
  }
  ...
  const autoreactStatus = await checkAutoreact(m.from);
  if (args[0] == "on") {
    if (autoreactStatus) {
      await doReact("❌");
      return m.reply(`The autoreact feature is already enabled!`);
    }
    await doReact("⚜️");
    await setAutoreact(m.from);
    await m.reply(`The autoreact feature has been enabled successfully!\n\nLadubug will react to messages with emojis.`);
  } else if (args[0] == "off") {
    if (!autoreactStatus) {
      await doReact("❌");
      return m.reply(`The autoreact feature is already disabled!`);
    }
    await doReact("⚜️");
    await delAutoreact(m.from);
    await m.reply(`The autoreact feature has been disabled successfully!`);
  } else {
    await doReact("❔");
    return m.reply(`Please provide an action: on or off.\n\nExample: ${prefix}autoreact on`);
  }
