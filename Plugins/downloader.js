    ...
    try {
      switch (inputCMD) {
        ...
        case "instadl":
          ...
          try {
            const res = await axios.get(
              "https://fantox001-scrappy-api.vercel.app/instadl?url=" +
                text,
              { timeout: 300000 }
            );
            const scrappedURL = res.data.videoUrl;
      ...
      *📌 File Format* : ${MDF[0].mime}
      ...
      Downloading...`;
            ...
            await doReact("❌");
            await m.reply(
              `Error occurred while downloading from Mediafire. Please try again later.`
            );
          }
          break;
        ...
        case "fbdl":
        case "facebookdl":
          if (!text || (!text.includes("fb") && !text.includes("facebook"))) {
            await doReact("❌");
            return m.reply(
              `Please provide a valid Facebook link!\n\nExample: *${prefix}fbdl put_link*`
            );
          ...
          try {
            ...
            await doReact("❌");
            await m.reply(
              `Video access denied! It's private or only the owner's friends can view it.`
            );
          }
