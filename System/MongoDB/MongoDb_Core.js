...
// userData.statics.getTotalUserCount = async function () {
...
async function getTotalUserCount() {
  ...
  return { count }; // Return an object with the 'count' property
}


 
 
// const { user, totalUserCount } = await getTotalUserCountAndUserInfo(userId);
...
// SET ANTI-WORD
async function setAntiword(groupID) {
  const group = await groupData.findOne({ id: groupID });
  if (!group) {
    await groupData.create({ id: groupID, antiword: true });
    return;
  }
  if (group.antiword) {
    return;
  }
  await groupData.findOneAndUpdate({ id: groupID }, { $set: { antiword: true } });
}

// CHECK ANTI-WORD STATUS
// Function to check if anti-word feature is enabled for a group
async function checkAntiword(groupID) {
  const group = await groupData.findOne({ id: groupID });
  if (!group) {
    return false;
  }
  return group.antiword;
}


// DELETE ANTI-WORD
async function delAntiword(groupID) {
  const group = await groupData.findOne({ id: groupID });
  if (!group) {
    await groupData.create({ id: groupID, antiword: false });
    return;
  }
  if (!group.antiword) {
    return;
  }
  await groupData.findOneAndUpdate({ id: groupID }, { $set: { antiword: false } });
}

// SET AUTOREACT
async function setAutoreact(groupID) {
  const group = await groupData.findOne({ id: groupID });
  if (!group) {
    await groupData.create({ id: groupID, autoreact: true });
    return;
  }
  if (group.autoreact) {
    return;
  }
  await groupData.findOneAndUpdate({ id: groupID }, { $set: { autoreact: true } });
}

// CHECK AUTOREACT STATUS
async function checkAutoreact(groupID) {
  const group = await groupData.findOne({ id: groupID });
  if (!group) {
    return false;
  }
  return group.autoreact;
}

// DELETE AUTOREACT
async function delAutoreact(groupID) {
  const group = await groupData.findOne({ id: groupID });
  if (!group) {
    await groupData.create({ id: groupID, autoreact: false });
    return;
  }
  if (!group.autoreact) {
    return;
  }
  await groupData.findOneAndUpdate({ id: groupID }, { $set: { autoreact: false } });
}
...
// SWICH OFF GROUP
async function offGroup(groupID) {
  const group = await groupData.findOne({ id: groupID });
  if (!group) {
    await groupData.create({ id: groupID, botSwitch: true });
    return;
  }
  if (group.offgroup) {
    return;
  }
  await groupData.findOneAndUpdate({ id: groupID }, { $set: { botSwitch: true } });
}

// CHECK SWICH GROUP STATUS
async function checkOffGroup(groupID) {
  const group = await groupData.findOne({ id: groupID });
  if (!group) {
    return false;
  }
  return group.offgroup;
}

// SWITCH ON GROUP
async function switchOnGroup(groupID) {
  const group = await groupData.findOne({ id: groupID });
  if (!group) {
    await groupData.create({ id: groupID, botSwitch: false });
    return;
  }
  if (!group.offgroup) {
    return;
  }
  await groupData.findOneAndUpdate({ id: groupID }, { $set: { botSwitch: false } });
}
...
async function getAllPlugins() {
 ...
 async function getAllUsersUsingBot() {
...
module.exports = {
  ...
  setAntiword, // ------------------ SET ANTIword
  checkAntiword, // ---------------- CHECK ANTIword STATUS
  delAntiword,
  setAutoreact,   //----------------- setantiword
  checkAutoreact,
  delAutoreact,
  getAllUsersUsingBot,
  offGroup,
